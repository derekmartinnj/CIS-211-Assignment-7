...
instr_field = BitField(26, 30) #OpCode
cond_field = BitField(22, 25) # treated as a small array of bits
reg_target_field = BitField(18, 21) #target register
reg_src1_field = BitField(14, 17) #source register 1
reg_src2_field = BitField(10, 13) #source register 2
offset_field = BitField(0, 9) # this field is treated as a signed integer
...
def decode(word: int) -> "Instruction":
    ...
    instr = Instruction(OpCode(instr_field.extract(word)),
                        CondFlag(cond_field.extract(word)),
                        reg_target_field.extract(word),
                        reg_src1_field.extract(word),
                        reg_src2_field.extract(word),
